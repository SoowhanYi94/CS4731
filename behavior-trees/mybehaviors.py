...
def makeAddNode(type, agent, parent, *args):
	node = makeNode(type, agent, *args)
...
def myBuildTree(agent):
	...
	root = None
	### YOUR CODE GOES BELOW HERE ###
	root = makeNode(Root, agent, "root")
	selector1 = makeNode(Selector, agent, "selector1")
	retreat = makeNode(Retreat, agent, 0.5, "retreat")
	hitpointDaemon = makeNode(HitpointDaemon, agent, 0.5, "hitpointDaemon")
	selector2 = makeNode(Selector, agent, "selector2")
	buffDaemon = makeNode(BuffDaemon, agent, 1, "buffDaemon")
	sequence1 = makeNode(Sequence, agent, "sequence1")
	chaseHero = makeNode(ChaseHero, agent, "chaseHero")
	killHero = makeNode(KillHero, agent, "killHero")
	sequence2 = makeNode(Sequence, agent, "sequence2")
	chaseMinion = makeNode(ChaseMinion, agent, "chaseMinion")
	killMinion= makeNode(KillMinion, agent, "killMinion")
	root.addChild(selector1)
	selector1.addChild(retreat)
	selector1.addChild(hitpointDaemon)
	hitpointDaemon.addChild(selector2)
	selector2.addChild(buffDaemon)
	selector2.addChild(sequence1)
	buffDaemon.addChild(sequence2)
	sequence1.addChild(chaseHero)
	sequence1.addChild(killHero)
	sequence2.addChild(chaseMinion)
	sequence2.addChild(killHero)
	### YOUR CODE GOES ABOVE HERE ###
	return root
...
class Root(BTNode):
	...
	def execute(self, delta = 0):
		...
		ret = self.getChild(0).execute(delta)
		enemies = self.agent.world.getEnemyNPCs(self.agent.getTeam())
		enemies = sorted(enemies, key = lambda x: distance(x.getLocation(),self.agent.getLocation()))
		## attack closest
		if self.agent.canfire:
			if not (enemies == None or len(enemies) <= 0):
				if distance(enemies[0].getLocation(), self.agent.getLocation()) <= BIGBULLETRANGE:
					self.agent.turnToFace(enemies[0].getLocation())
					self.agent.shoot()
					return True
		## area attack if it can
		if self.agent.canAreaEffect:
			if distance(enemies[0].getLocation(), self.agent.getLocation()) <= AREAEFFECTRANGE:
		...
		if self.agent.canDodge():
			...
			for b in bullets:
				if b.getOwner().getTeam() != self.agent.getTeam() and distance(self.agent.getLocation(), b.getLocation()) < self.agent.getRadius()*3:
					self.agent.dodge()
					return True
		return ret
